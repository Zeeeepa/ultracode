...
| `countTokens` | function | Функция для подсчёта частоты каждого токена в тексте | [→ tokenizer.ts:351-355] |
...
| `extractNgrams` | function | Функция для извлечения последовательностей слов из текста | [→ tokenizer.ts:367-375] |
| `extractTopTerms` | function | Функция для извлечения главных терминов из документов | [→ tfidf.ts:192-198] |
| `QueryExpander` | class | Класс для расширения запросов с использованием совместного появления | [→ query-expander.ts:56-216] |
...
| `tokenize` | function | Функция для разделения текста на токены с фильтрацией | [→ tokenizer.ts:293-335] |
| `tokenizeUnique` | function | Функция для получения уникальных токенов из текста | [→ tokenizer.ts:341-343] |
