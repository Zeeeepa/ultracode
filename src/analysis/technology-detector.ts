...
export class TechnologyDetector {
  ...
  private async detectFromImports(): Promise<FrameworkInfo[]> {
    ...
    const addFile = (key: string, file: string) => {
      let s = importFiles.get(key);
      if (!s) {
        s = new Set();
        importFiles.set(key, s);
      }
      s.add(file);
    };
