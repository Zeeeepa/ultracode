...
Language-agnostic custom detectors for identifying code quality anti-patterns across multiple programming languages.
...
| `checkDeepNesting` | function | Detects code blocks exceeding nesting depth threshold | [→ common.ts:31-42] |
| `checkEmptyCatch` | function | Detects try-catch blocks with empty handlers swallowing exceptions | [→ typescript.ts:30-42] |
| `checkGodFunction` | function | Detects functions with high cyclomatic or line complexity | [→ common.ts:11-26] |
...
| `checkLargeClass` | function | Detects classes with excessive methods and code size | [→ common.ts:108-108] |
...
| `checkNoDocumentation` | function | Detects public entities lacking documentation blocks | [→ common.ts:61-79] |
...
| `checkSmallFocusedFunction` | function | Detects well-structured short focused function pattern | [→ common.ts:87-91] |
...
| `checkTooManyParams` | function | Detects functions with more than seven parameters | [→ common.ts:47-56] |
