...
| `CustomDetectorResult` | interface | Interface for custom detector function result | [→ types.ts:162-197] |
...
| `PatternMatch` | interface | Interface for detected pattern match result | [→ types.ts:126-153] |
| `PatternRegistry` | class | Indexes pattern definitions by language for efficient lookup | [→ pattern-registry.ts:90-112] |
| `PatternScanOptions` | interface | Interface for scan configuration and filtering options | [→ types.ts:141-171] |
| `PatternScanResult` | interface | Interface for complete scan results with summary | [→ types.ts:126-153] |
...
| `StructuralCandidate` | interface | Interface for pattern candidate before validation | [→ types.ts:162-197] |
| `StructuralCriteria` | interface | Interface for structural pattern matching requirements | [→ types.ts:24-137] |
