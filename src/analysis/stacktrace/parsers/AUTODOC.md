...
Collection of language-specific stacktrace parsers for eight programming languages with standardized frame extraction
...
| `GoStacktraceParser` | class | Extracts goroutine context and panic frames from Go dumps | [→ go-parser.ts:23-101] |
| `JavaScriptStacktraceParser` | class | Parses V8-style async-aware frames with positions | [→ javascript-parser.ts:17-83] |
| `JvmStacktraceParser` | class | Parses Java frames with recursive exception chain handling | [→ jvm-parser.ts:15-92] |
| `NativeStacktraceParser` | class | Detects GDB, ASAN, and macOS formats for native code | [→ native-parser.ts:21-110] |
| `PythonStacktraceParser` | class | Parses Python exception frames with inverted index ordering | [→ python-parser.ts:15-67] |
| `RustStacktraceParser` | class | Extracts backtrace frames with crate and type information | [→ rust-parser.ts:15-122] |
