 ...
 */
export async function handleFaissLoadFromDump(
  request: { dumpDir: string; dimensions: number },
  ctx: FaissHandlerContext,
): Promise<void> {
  const { faissIndex, state, log, sendResponse, sendError } = ctx;
 ...
 */
export async function handleFaissLoadWorkerDump(
  request: { workerId: string; dimensions: number },
  ctx: FaissHandlerContext,
): Promise<void> {
  const { faissIndex, state, log, sendResponse, sendError } = ctx;
