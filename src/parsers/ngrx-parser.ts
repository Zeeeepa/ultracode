...
export function buildNgRxRelationships(analysis: NgRxAnalysis): NgRxRelationship[] {
  ...
  const actionByName = new Map<string, NgRxAction>();
  const actionByType = new Map<string, NgRxAction>();

  // Index actions by name and type
  for (const action of analysis.actions) {
    actionByName.set(action.name, action);
    if (action.type) {
      actionByType.set(action.type, action);
    }
  }
