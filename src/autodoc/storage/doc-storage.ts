...
export class DocStorage {
  ...
  async initialize(): Promise<void> {
    ...
    // AutoDoc data is NOT regeneratable (LLM/human-written docs, changelogs).
    // WAL mode: concurrent reads during generation, NORMAL sync protects from data loss.
    // Synced with versioning.db PRAGMA strategy (same rationale: non-regeneratable data).
    await this.client.execute("PRAGMA journal_mode = WAL");
    await this.client.execute("PRAGMA synchronous = NORMAL");
    await this.client.execute("PRAGMA cache_size = -65536"); // 64MB
    await this.client.execute("PRAGMA busy_timeout = 5000");
    await this.client.execute("PRAGMA temp_store = MEMORY");
    await this.client.execute("PRAGMA mmap_size = 268435456"); // 256MB
  ...
  private async createTables(): Promise<void> {
    ...
    // Migration: add source_hash column for Zig-compatible change detection
    // Format: "hash:count:loc", prefixed with "enriched|", "synced|", or "edited"
    await this.client.execute(`ALTER TABLE doc_entities ADD COLUMN source_hash TEXT NOT NULL DEFAULT ''`).catch(() => {
  ...
  async createDoc(doc: Omit<DocEntity, "id" | "createdAt" | "updatedAt">): Promise<DocEntity> {
    ...
    await this.client!.execute({
      sql: `INSERT INTO doc_entities (id, type, file_path, section, title, content, tags, auto_generated, confidence, last_sync, source_hash, project_hash, branch_name, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        entity.id,
        entity.type,
  ...
  async updateDoc(
    ...
    await this.client!.execute({
      sql: `UPDATE doc_entities
            SET type = ?, title = ?, content = ?, tags = ?, auto_generated = ?, confidence = ?, last_sync = ?, source_hash = ?, updated_at = ?
            WHERE id = ?`,
      args: [
        updated.type,
        updated.title,
  ...
  // ---------------------------------------------------------------------------
  // Zig-compat: entity-based lookups for batch autodoc
  // ---------------------------------------------------------------------------

  /**
   * Get a doc by its entity_id (stored in doc ID as "doc::dir:{path}").
   * Used by batch autodoc to find existing directory docs.
   */
  async getDocByEntityId(entityId: string): Promise<DocEntity | null> {
    this.ensureReady();
    // Entity ID "dir:src/tools" maps to doc ID "doc::dir:src/tools"
    const docId = `doc::${entityId}`;
    return this.getDoc(docId);
  }

  /**
   * Get the source_hash for an entity without loading full content.
   * Returns null if no doc exists for this entity.
   */
  async getSourceHash(entityId: string): Promise<string | null> {
    this.ensureReady();
    const docId = `doc::${entityId}`;
    const result = await this.client!.execute({
      sql: `SELECT source_hash FROM doc_entities WHERE id = ? AND project_hash = ? AND branch_name = ?`,
      args: [docId, this.currentContext.projectHash, this.currentContext.branchName],
    });
    if (result.rows.length === 0) return null;
    return (result.rows[0] as unknown as { source_hash: string }).source_hash || null;
  }

  /**
   * Upsert a directory doc with source_hash (for batch autodoc).
   * Creates or updates based on entity ID.
   */
  async upsertDirectoryDoc(entityId: string, content: string, sourceHash: string, title: string): Promise<void> {
    this.ensureReady();
    const docId = `doc::${entityId}`;
    const now = Date.now();
    await this.client!.execute({
      sql: `INSERT INTO doc_entities (id, type, file_path, section, title, content, tags, auto_generated, confidence, last_sync, source_hash, project_hash, branch_name, created_at, updated_at)
            VALUES (?, 'module_index', ?, NULL, ?, ?, '[]', 1, 1.0, ?, ?, ?, ?, COALESCE((SELECT created_at FROM doc_entities WHERE id = ?), ?), ?)
            ON CONFLICT(id) DO UPDATE SET content = excluded.content, source_hash = excluded.source_hash, title = excluded.title, updated_at = excluded.updated_at`,
      args: [
        docId,
        entityId,
        title,
    ...
    const statements: InStatement[] = docs.map((doc) => {
      ...
      return {
        sql: `INSERT OR REPLACE INTO doc_entities
              (id, type, file_path, section, title, content, tags, auto_generated, confidence, last_sync, source_hash, created_at, updated_at)
