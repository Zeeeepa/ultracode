...
#   - Creates semantic-config.json in .ultrasharp/
#
# Usage:
...
if (!$SkipGpuDetection) {
    ...
    $detectScript = Join-Path $ScriptDir "detect-gpu-architecture.ps1"
...
if ($selectedProvider -eq "memory") {
    ...
    # Save config to project .ultrasharp directory
    if (!(Test-Path $OutputConfigDir)) {
    ...
    Write-Success "Configuration saved to: $OutputConfigFile"
    Write-Host ""
    exit 0
}
...
foreach ($model in $availableModels) {
    ...
    $langLabel = if ($model.language -eq "multi") { "Multilingual" } elseif ($model.language -eq "code") { "Code" } else { "English" }
...
if ($selectedProvider -eq "tei") {
    ...
    $setupScript = Join-Path $ScriptDir "setup-tei.ps1"
    if (Test-Path $setupScript) {
        & $setupScript -ModelId $selectedModel.model_id -Architecture $architecture
        if ($LASTEXITCODE -ne 0) {
            Write-Host ""
            Write-Host "TEI setup encountered issues" -ForegroundColor Yellow
            Write-Host "  You can run it manually: .\scripts\setup-tei.ps1 -ModelId $($selectedModel.model_id) -Architecture $architecture" -ForegroundColor Gray
        }
    } else {
    ...
    Write-Host ""
    Write-Host "Next: Restart your MCP client to enable semantic mode" -ForegroundColor Yellow
    Write-Host ""
} elseif ($selectedProvider -eq "ollama") {
    Write-Host "Installing Ollama and downloading model..." -ForegroundColor Yellow
    Write-Host ""

    $setupScript = Join-Path $ScriptDir "setup-ollama.ps1"
    if (Test-Path $setupScript) {
        & $setupScript -ModelId $selectedModel.model_id
        if ($LASTEXITCODE -ne 0) {
            Write-Host ""
            Write-Host "Ollama setup encountered issues" -ForegroundColor Yellow
            Write-Host "  You can run it manually: .\scripts\setup-ollama.ps1 -ModelId $($selectedModel.model_id)" -ForegroundColor Gray
        }
    } else {
