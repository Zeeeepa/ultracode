 ...
 *
 * After installation:
 *   - Windows: %LOCALAPPDATA%\UltraCode\ovms\ovms.exe
 *   - Linux: ~/.local/share/ultracode/ovms/ovms
 */
...
function getOVMSDir(): string {
  if (process.platform === "win32") {
    ...
    return join(localAppData, "UltraCode", "ovms");
  } else {
    const home = process.env.HOME || "/tmp";
    return join(home, ".local", "share", "ultracode", "ovms");
  }
}
...
function getModelsDir(): string {
  if (process.platform === "win32") {
    const localAppData =
      process.env.LOCALAPPDATA || join(process.env.USERPROFILE || "", "AppData", "Local");
    return join(localAppData, "UltraCode", "models");
  } else {
    const home = process.env.HOME || "/tmp";
    return join(home, ".local", "share", "ultracode", "models");
  }
}
