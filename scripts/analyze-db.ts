...
async function analyze() {
  ...
  const shadowSchema = await client.execute(
    "SELECT sql FROM sqlite_master WHERE name = 'idx_emb_vec_fad95849_shadow'",
  );
  ...
  console.log("\n=== SHADOW ROW SIZE ===");
  try {
    const shadowSample = await client.execute(
      "SELECT index_key, LENGTH(data) as data_len FROM idx_emb_vec_fad95849_shadow LIMIT 5",
    );
    for (const row of shadowSample.rows) {
      console.log(`Row ${row.index_key}: ${row.data_len} bytes`);
    }
    const avgSize = await client.execute(
      "SELECT AVG(LENGTH(data)) as avg_len FROM idx_emb_vec_fad95849_shadow",
    );
  ...
  console.log(
    `Free pages: ${freePages} / ${totalPages} (${((freePages / totalPages) * 100).toFixed(1)}%)`,
  );
