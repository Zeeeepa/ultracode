...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "[OK] Ollama is already installed: $version" -ForegroundColor Green
    }
}
catch {
    # Ollama not found
...
if (!$ollamaInstalled) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "[WARNING] Please restart this script to continue" -ForegroundColor Yellow
        Write-Host ""
...
if ([string]::IsNullOrWhiteSpace($ModelId)) {
    ...
    if ($existingModels) {
        Write-Host "[OK] Model '$ModelId' is already downloaded" -ForegroundColor Green
        Write-Host ""
    }
    else {
