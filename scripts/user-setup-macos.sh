...
# - Clang (Xcode Command Line Tools, for C/C++ parsing)
# - Java (for Java/Kotlin parsing)
# - shfmt (for Bash parsing)
#
# Usage: ./user-setup-macos.sh
#        ./user-setup-macos.sh --auto  (auto-install without prompts)
...
check_homebrew() {
    if ! command_exists brew; then
        ...
        if ask_install "Homebrew" "https://brew.sh/"; then
...
install_with_brew_cask() {
    ...
    if check_homebrew; then
        print_info "Installing $name via Homebrew Cask..."
...
# 2. Python (usually pre-installed on macOS)
...
if [[ -n "$python_cmd" ]]; then
    ...
    if ask_install "Python" "https://www.python.org/downloads/"; then
        install_with_brew "python@3.12" "Python"
    fi
fi
...
# 5. Clang (via Xcode Command Line Tools)
...
if command_exists clang; then
    ...
    if ask_install "Xcode Command Line Tools" "https://developer.apple.com/xcode/"; then
        print_info "Installing Xcode Command Line Tools..."
        xcode-select --install 2>/dev/null || print_warning "Already installing or installed"
    fi
fi
...
if command_exists java; then
    ...
    if ask_install "Java" "https://adoptium.net/"; then
        install_with_brew_cask "temurin" "Java (Temurin)"
    fi
fi
...
if command_exists shfmt; then
    ...
    if ask_install "shfmt" "https://github.com/mvdan/sh/releases"; then
        install_with_brew "shfmt" "shfmt"
    fi
fi

# 8. Bash/Zsh (always available on macOS)
...
if [[ -n "$ZSH_VERSION" ]]; then
    ...
    shell_version=$(bash --version | head -1)
fi
print_success " OK ($shell_version)"
results["Shell"]="OK"
