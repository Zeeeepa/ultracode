...
MCP-сервер для ИИ-агентов, работающих с кодом. Строит полный граф структуры кода (сущности, связи, control flow, complexity) и семантический векторный индекс. ИИ-агенты обращаются к графу вместо чтения файлов — и получают точные, исчерпывающие ответы со ссылками на строки кода.

### Почему это важно

Без структурного индекса ИИ-агент исследует кодовую базу по цепочке: **grep → читает файл → идёт по импортам → снова grep → читает ещё файлы**. Каждый шаг стоит токенов и времени. Пропущенные связи ведут к неполным исправлениям. Агент ломает код, проверяет, чинит, снова ломает — цикл, который может повториться 10-20 раз для одной задачи.

С UltraCode тот же агент делает **один MCP-вызов** и получает все затронутые сущности, их связи, вызывающий код и анализ влияния — в одном ответе. Без цикла чтения файлов, без пропущенных зависимостей.
...
> Подробности по каждому языку: [.autodoc/features/language-parsers.md](.autodoc/features/language-parsers.md)
...
**Сейчас поддерживается:** Zig — полное извлечение сущностей, связей, control flow и метрик сложности. Swift — полные сущности включая SwiftUI property wrappers, раздельное наследование/conformance, control flow.
