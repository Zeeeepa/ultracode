...
const createMockParsedEntity = (name: string, type: ParsedEntity["type"] = "function"): ParsedEntity => ({
  name,
  type,
  location: {
...
describe("IndexerAgent", () => {
  ...
  describe("Entity Indexing", () => {
    ...
    test("should index entities from parsed results", async () => {
      ...
      const result = await agent.indexEntities(parseResult.entities, parseResult.filePath);
      ...
      const entities: ParsedEntity[] = [
        {
          ...
          children: [createMockParsedEntity("childMethod", "method")],
        },
        {
  ...
  describe("Error Handling", () => {
    ...
    test("should handle database errors gracefully", async () => {
      ...
      await expect(agent.indexEntities(entities, "/test/error.ts")).rejects.toThrow();
    });
