...
`imports`, `calls`, `extends`, `implements`, `references`, `contains`, `uses`, `produces_api`, `consumes_api`, `generated_from`
...
**Детекция устаревшего Swagger**: Для проектов со Swagger/OpenAPI проверка состояния также сообщает, когда сгенерированный код рассинхронизирован:
...
```typescript
{
  ...
  swaggerHealth: {
    swaggerFilesFound: 2,
    activeContracts: 1,
    staleWarnings: ["Сгенерированный код может быть рассинхронизирован: swagger.json обновлён после generated/api-client.ts"]
  }
}
```
