...
**Contract Impact** (for projects with Swagger/OpenAPI):

When impacted entities have `produces_api`, `consumes_api`, or `generated_from` relationships, the response includes a `contractImpact` section:
...
> Learn more: [swagger.md](swagger.md)

---

## analyze_swagger_impact

Analyze impact of Swagger/OpenAPI spec changes — affected controllers, generated clients, generated types, and breaking change risk.

### Parameters

| Parameter | Type | Required | Description |
|----------|------|----------|-------------|
| `swaggerFile` | string | no | Path to swagger file (auto-detected) |
| `schemaName` | string | no | Specific schema name |
