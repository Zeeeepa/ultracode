# FAISS Prebuilt Binary - macOS Apple Silicon (ARM64)

This directory contains the prebuilt `faiss-node` native addon for **macOS Apple Silicon (M1/M2/M3)** and **Node 24 (ABI v137)**.
...
## How to build (for contributors with macOS)
...
```bash
node -e "require('./external-libs/faiss-darwin-arm64/faiss-node.node'); console.log('✓ Works!')"
```
