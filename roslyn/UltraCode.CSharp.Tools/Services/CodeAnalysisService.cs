// Portions of this code are derived from RoslynMCP by Christopher Arquiza
// https://github.com/carquiza/RoslynMCP
// Original code licensed under MIT License
// Modifications: Added caching, performance optimizations, FastSymbolIndex integration
...
) : ICodeAnalysisService
{
    ...
    private string GetSolutionHash()
    {
        var solution = GetCurrentSolutionOrThrow();

        // Use solution file path + last write time as hash
        if (!string.IsNullOrEmpty(solution.FilePath) && File.Exists(solution.FilePath))
        {
            var lastWriteTime = File.GetLastWriteTimeUtc(solution.FilePath).Ticks;
            return $"{solution.FilePath}_{lastWriteTime}".GetHashCode().ToString("X");
        }

        // Fallback: use solution ID
    ...
    )
    {
        ...
        if (_cacheService != null)
        {
            ...
            )
            {
                ...
                // For now, we skip cache for this operation as it's complex
        ...
        if (_cacheService != null && results != null)
        {
            ...
            foreach (var referencedSymbol in results)
            {
            ...
            _cacheService.SetCached(solutionHash, "FindReferences", parameters, cachedResult);
        }
    ...
    private static ITypeSymbol? ProcessFieldReference(
    ...
    private static ITypeSymbol? ProcessPropertyReference(
    ...
    private static ITypeSymbol? ProcessMethodReference(
    ...
    private static ITypeSymbol? ProcessEventReference(
