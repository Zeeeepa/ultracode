...
using UltraCode.CSharp.Tools.Semantic.Embedding.Providers;
using UltraCode.CSharp.Tools.Semantic.GPU;

namespace UltraCode.CSharp.Tools.Semantic.Embedding;
...
public sealed class EmbeddingProviderFactory
{
    ...
    public async Task<IEmbeddingProvider> CreateAsync(CancellationToken cancellationToken = default)
    {
        ...
        // Auto-detect mode: select provider based on GPU
        if (providerType == "auto" && _options.AutoDetectGPU)
        {
            _logger.LogInformation("[EmbeddingFactory] Auto-detecting GPU capabilities...");
            var gpuInfo = await _gpuDetection.DetectAsync(cancellationToken);
