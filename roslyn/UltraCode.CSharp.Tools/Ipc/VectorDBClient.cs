...
namespace UltraCode.CSharp.Tools.Ipc;
...
public sealed class VectorDBClient : IDisposable {
    ...
    public async Task ConnectAsync(CancellationToken cancellationToken = default) {
        // Если VectorDB уже запущен, сигнализируем wake-up event перед подключением
        // Это разбудит VectorDB из idle mode (EcoQoS) и восстановит ThreadPool
        if (IsVectorDBProcessRunning()) {
            SignalWakeUpEvent();
            await Task.Delay(150, cancellationToken); // Даём время на wake-up
        }
        ...
        try {
            ...
            // Таймаут - проверяем, может VectorDB в idle mode и просто медленно отвечает
            _pipeClient?.Dispose();
            _pipeClient = null;

            if (IsVectorDBProcessRunning()) {
                // Процесс существует но медленно отвечает - retry с большим таймаутом
                // (на случай если wake-up event не сработал)
                try {
                    _logger.LogInformation("[VectorDB] VectorDB process slow to respond, retrying...");
                    SignalWakeUpEvent(); // Повторная попытка wake-up
                    await Task.Delay(200, cancellationToken);

                    _pipeClient = new NamedPipeClientStream(".", PipeName, PipeDirection.InOut, PipeOptions.Asynchronous);
                    await _pipeClient.ConnectAsync(10000, cancellationToken); // 10 секунд после wake-up
                    InitializeStreams();
                    _logger.LogInformation("[VectorDB] Connected to VectorDB process (after wake-up)");
                    return;
                } catch (TimeoutException) {
                    // Даже после wake-up не отвечает - процесс завис, убиваем и запускаем новый
                    _pipeClient?.Dispose();
                    _pipeClient = null;
                    _logger.LogWarning("[VectorDB] Process not responding after wake-up signal, killing and restarting...");
                    KillVectorDBProcess();
                }
            }

            _logger.LogInformation("[VectorDB] No responding process found, starting new VectorDB...");
     ...
     /// Проверяет, запущен ли процесс VectorDB
     /// </summary>
    private bool IsVectorDBProcessRunning() {
        try {
            var processes = Process.GetProcessesByName("UltraSharpTools.VectorDB");
    ...
    /// <summary>
    /// Завершает все процессы VectorDB
    /// </summary>
    private void KillVectorDBProcess() {
        try {
            var processes = Process.GetProcessesByName("UltraSharpTools.VectorDB");
            foreach (var p in processes) {
                try {
            ...
            _logger.LogWarning(ex, "[VectorDB] Failed to kill VectorDB process");
        }
    }
    /// <summary>
    /// Сигнализирует Named Event для пробуждения VectorDB из idle mode.
    /// Это kernel-level операция, не зависит от состояния ThreadPool VectorDB.
    /// Работает только на Windows (Named Events - Windows-специфичный механизм).
    /// </summary>
    private void SignalWakeUpEvent() {
        // Named Events - Windows-specific, на других платформах пропускаем
        if (!OperatingSystem.IsWindows())
            return;

        const string WakeUpEventName = "UltraSharpTools_VectorDB_WakeUp";
        try {
            // Открываем существующий event (VectorDB должен был его создать)
            using var wakeUpEvent = EventWaitHandle.OpenExisting(WakeUpEventName);
            wakeUpEvent.Set();
            _logger.LogDebug("[VectorDB] Wake-up signal sent to VectorDB");
        } catch (WaitHandleCannotBeOpenedException) {
            // Event не существует - VectorDB ещё не создал его или старая версия
            _logger.LogDebug("[VectorDB] Wake-up event not found (VectorDB may be initializing or old version)");
        } catch (Exception ex) {
            _logger.LogWarning(ex, "[VectorDB] Failed to signal wake-up event");
        }
    }
}
