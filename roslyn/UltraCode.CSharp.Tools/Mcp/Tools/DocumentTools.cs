...
using UltraCode.CSharp.Tools.Mcp.Tools;
...
public static class DocumentTools
{
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(ReadRawFromRoslynDocument),
                    cancellationToken
                );
                ...
                try
                {
                    var (contents, lines) = await documentOperations.ReadFileAsync(
                        filePath,
                        true,
                        cancellationToken
                    );
                    ...
                    when (ex is IOException or UnauthorizedAccessException or SecurityException)
                {
                    logger.LogError(
                        ex,
                        "Failed to read file due to access restrictions: {FilePath}",
                        filePath
                    );
                    throw new McpException(
                        $"Failed to read file due to access restrictions: {ex.Message}"
                    );
                }
                catch (Exception ex)
    ...
    public static async Task<string> CreateRoslynDocument(
        ...
        [Description(
            "The content to write to the file. For C#, omit indentation to save tokens. Code will be auto-formatted."
        )]
            string content,
    ...
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                {
                    throw new McpException(
                        $"File already exists at {filePath}. Use '{ToolHelpers.SharpToolPrefix}{nameof(ReadRawFromRoslynDocument)}' to understand its contents. Then you can use '{ToolHelpers.SharpToolPrefix}{nameof(OverwriteRoslynDocument)}' if you decide to overwrite what exists."
                    );
                }
                ...
                {
                    throw new McpException(
                        $"Cannot create file at {filePath}. Path is not writable."
                    );
                }

                string finalCommitMessage =
                    $"Create {Path.GetFileName(filePath)}: " + commitMessage;
                try
                {
                    ...
                    var success = await documentOperations.WriteFileAsync(
                        filePath,
                        content,
                        false,
                        cancellationToken,
                    ...
                    {
                        throw new McpException(
                            $"Failed to create file at {filePath} for unknown reasons."
                        );
                    }
                    ...
                    when (ex is IOException or UnauthorizedAccessException or SecurityException)
                {
                    logger.LogError(
                        ex,
                        "Failed to create file due to IO or access restrictions: {FilePath}",
                        filePath
                    );
                    throw new McpException(
                        $"Failed to create file due to IO or access restrictions: {ex.Message}"
                    );
                }
                catch (Exception ex)
                    when (!(ex is McpException || ex is OperationCanceledException))
    ...
    [
        ...
        Description(
            $"Overwrites an existing document file with the specified content. You must use 'read_file' first."
        )
    ]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(OverwriteRoslynDocument),
                    cancellationToken
                );
                content = content.TrimBackslash();
                ...
                {
                    throw new McpException(
                        $"Cannot write to file at {filePath}. Path is not writable."
                    );
                }

                string finalCommitMessage =
                    $"Update {Path.GetFileName(filePath)}: " + commitMessage;
                try
                {
                    ...
                    if (documentOperations.FileExists(filePath))
                    {
                        (originalContent, _) = await documentOperations.ReadFileAsync(
                            filePath,
                            false,
                            cancellationToken
                        );
                    ...
                    if (isCodeFile)
                    {
                        existingDocument = solutionManager
                            .CurrentSolution?.Projects.SelectMany(p => p.Documents)
                            .FirstOrDefault(d =>
                                d.FilePath != null
                                && d.FilePath.Equals(filePath, StringComparison.OrdinalIgnoreCase)
                    ...
                    {
                        logger.LogInformation(
                            "Document found in solution, updating through workspace API: {FilePath}",
                            filePath
                        );
                        ...
                        await codeModificationService.ApplyChangesAsync(
                            solution,
                            cancellationToken,
                            finalCommitMessage
                        );
                    ...
                    else
                    {
                        ...
                        {
                            throw new McpException(
                                $"Failed to overwrite file at {filePath} for unknown reasons."
                            );
                        }
                        ...
                        else
                        {
                            ...
                            {
                                projectStatus =
                                    $"and was detected in project {document.Project.Name}";
                                ...
                                {
                                    resultMessage =
                                        $"Overwrote file {filePath} ({projectStatus}), but compilation issues were detected:\n\n{errorMessages}";
                    ...
                    when (ex is IOException or UnauthorizedAccessException or SecurityException)
                {
                    logger.LogError(
                        ex,
                        "Failed to overwrite file due to IO or access restrictions: {FilePath}",
                        filePath
                    );
                    throw new McpException(
                        $"Failed to overwrite file due to IO or access restrictions: {ex.Message}"
                    );
                }
                catch (Exception ex)
                    when (!(ex is McpException || ex is OperationCanceledException))
    ...
    private static string AddDiffToResult(
        string resultMessage,
        string oldContent,
        string newContent
    )
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(ReadTypesFromRoslynDocument),
                    cancellationToken
                );
                ...
                var documentId = solutionManager
                ...
                {
                    throw new McpException(
                        $"Could not get semantic model for document: {filePath}"
                    );
                }

                var typeNodes = syntaxRoot.DescendantNodes().OfType<TypeDeclarationSyntax>();
                ...
                foreach (var typeNode in typeNodes)
                {
                    ...
                    if (typeNode.Parent is BaseNamespaceDeclarationSyntax or CompilationUnitSyntax)
                    {
                        ...
                        if (declaredSymbol is INamedTypeSymbol declaredNamedTypeSymbol)
                        {
                            ...
                            string fullyQualifiedName = declaredNamedTypeSymbol.ToDisplayString(
                                SymbolDisplayFormat.FullyQualifiedFormat
                            );
                            var resolvedNamedTypeSymbol =
                                await solutionManager.FindRoslynNamedTypeSymbolAsync(
                                    fullyQualifiedName,
                                    cancellationToken
                                );
                            INamedTypeSymbol symbolToProcess =
                                resolvedNamedTypeSymbol ?? declaredNamedTypeSymbol;
                            ...
                            if (
                                symbolToProcess.TypeKind != TypeKind.Error
                                && symbolToProcess.ContainingAssembly != null
                            )
                            {
                                result.Add(
                                    await AnalysisTools.BuildRoslynSubtypeTreeAsync(
                                        symbolToProcess,
                                        codeAnalysisService,
                                        cancellationToken
                                    )
                            ...
                            {
                                logger.LogWarning(
                                    "Skipping symbol {SymbolName} from file {FilePath} as it could not be properly resolved to a valid named type symbol with an assembly context. Resolved: {ResolvedIsNull}, Declared TypeKind: {DeclaredTypeKind}, Declared AssemblyNull: {DeclaredAssemblyIsNull}",
                                    fullyQualifiedName,
                                    filePath,
                                    resolvedNamedTypeSymbol == null,
