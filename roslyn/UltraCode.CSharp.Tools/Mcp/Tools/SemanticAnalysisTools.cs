...
using UltraCode.CSharp.Tools.Semantic;
...
public static partial class SemanticAnalysisTools
{
    ...
    [Description(
        "Find semantically similar code using natural language keywords or code snippets. "
    ...
    public static async Task<object> SemanticSearch(
        ...
        [Description(
            "Natural language query (e.g., 'validate email address') or code snippet to search for"
        )]
            string query,
        [Description(
            "Search scope: 'solution' (all projects), 'project:<name>', or 'methods'/'classes' to filter by type"
        )]
            string scope = "solution",
        [Description("Number of results to return (1-50)")] int topK = 10,
        [Description(
            "Minimum similarity threshold (0.0-1.0). Higher values return only very similar matches."
        )]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                logger.LogInformation(
                    "Executing {Tool} for query: '{Query}', scope: {Scope}, topK: {TopK}, minSimilarity: {MinSimilarity}",
                    nameof(SemanticSearch),
                    query,
                    scope,
                    topK,
                ...
                {
                    results = await searchService.FindSimilarMethodsAsync(
                        query,
                        topK,
                        minSimilarity,
                        cancellationToken
                ...
                {
                    results = await searchService.FindSimilarClassesAsync(
                        query,
                        topK,
                        minSimilarity,
                        cancellationToken
                ...
                else if (scope.StartsWith("project:", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    results = await searchService.FindSimilarCodeAsync(
                        query,
                        topK,
                        minSimilarity,
                        cancellationToken
                ...
                return ToolHelpers.ToJson(
                    new
                    {
                        ...
                        indexMetrics = new
                        {
                            ...
                            embeddingCacheHitRate = Math.Round(
                                metrics.EmbeddingMetrics.CacheHitRate * 100,
                                2
                            ) + "%",
                            avgEmbedTimeMs = Math.Round(
                                metrics.EmbeddingMetrics.AverageEmbedTimeMs,
                                2
                            ),
                        },
                        results = results
    ...
    [Description(
        "Compare two code entities (methods/classes) semantically to determine if their behavior changed. "
    ...
    public static async Task<object> SemanticDiff(
        ...
        ILogger<SemanticAnalysisToolsLogCategory> logger,
        [Description("Fully qualified name of first entity (before changes)")] string beforeFqn,
        [Description(
            "Fully qualified name of second entity (after changes). Can be same as beforeFqn if modified in place."
        )]
            string afterFqn,
        [Description(
            "Include implementation details in comparison (default: false, focuses on high-level behavior)"
        )]
    ...
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                logger.LogInformation(
                    "Executing {Tool}: comparing '{Before}' vs '{After}'",
                    nameof(SemanticDiff),
                    beforeFqn,
                    afterFqn
                );
                ...
                // Get symbols
                var beforeSymbol = await ToolHelpers.GetRoslynSymbolOrThrowAsync(
                    solutionManager,
                    beforeFqn,
                    cancellationToken
                );
                var afterSymbol = await ToolHelpers.GetRoslynSymbolOrThrowAsync(
                    solutionManager,
                    afterFqn,
                    cancellationToken
                );
                ...
                // Extract code
                var beforeCode = await GetSymbolCodeAsync(
                    beforeSymbol,
                    solutionManager,
                    cancellationToken
                );
                var afterCode = await GetSymbolCodeAsync(
                    afterSymbol,
                    solutionManager,
                    cancellationToken
                );
                ...
                var similarity = await ComputeSemanticSimilarityAsync(
                    searchService,
                    beforeCode,
                    afterCode,
                    cancellationToken
                ...
                if (includeImplementationDetails)
                {
                    ...
                    codeMetrics = new
                    {
                        ...
                        sizeDeltaPercent = beforeLines > 0
                            ? Math.Round((afterLines - beforeLines) * 100.0 / beforeLines, 2)
                            : 0,
                    };
                }
    ...
    [Description(
        "Find duplicate or similar code blocks across the codebase using semantic analysis. "
    ...
    public static async Task<object> DetectCodeClones(
        ...
        ILogger<SemanticAnalysisToolsLogCategory> logger,
        [Description(
            "Minimum similarity threshold (0.0-1.0) to consider code as clones. 0.85 recommended for duplicates, 0.7 for similar patterns."
        )]
            float minSimilarity = 0.85f,
        [Description(
            "Clone detection mode: 'semantic' (ML-based, default), 'exact' (identical code), or 'similar' (flexible matching)"
        )]
            string mode = "semantic",
        [Description(
            "Only analyze methods and properties, skip classes (default: true for faster analysis)"
        )]
            bool membersOnly = true,
        [Description("Maximum number of clone groups to return (default: 20)")] int maxGroups = 20,
        CancellationToken cancellationToken = default
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                logger.LogInformation(
                    "Executing {Tool}: mode={Mode}, minSimilarity={MinSimilarity}, membersOnly={MembersOnly}, maxGroups={MaxGroups}",
                    nameof(DetectCodeClones),
                    mode,
                    minSimilarity,
                    membersOnly,
                ...
                var entities = await GetAllCodeEntitiesAsync(
                    solutionManager,
                    membersOnly,
                    cancellationToken
                );
    ...
    private static async Task<string> GetSymbolCodeAsync(
        ISymbol symbol,
        ISolutionManager solutionManager,
    ...
    )
    {
        ...
        var matches = await searchService.FindSimilarCodeAsync(
            code1,
            limit: 1,
            minSimilarity: 0.0f,
            cancellationToken
        ...
        var reverseMatches = await searchService.FindSimilarCodeAsync(
            code2,
            limit: 1,
            minSimilarity: 0.0f,
            cancellationToken
    ...
    private static string[] GetChangeRisks(string category, float similarity)
    {
        return category switch
        {
            ...
            "moderate_change" => new[]
            {
                ...
                "Check for breaking changes in callers",
            },
            "major_change" => new[]
            {
            ...
            _ => new[]
            {
                "Critical: Complete behavioral change",
    ...
    private static string[] GetChangeRecommendations(string category, float similarity)
    {
        return category switch
        {
            ...
            "refactoring" => new[]
            {
                ...
                "Consider adding comments explaining refactoring",
            },
            "minor_change" => new[]
            {
            ...
            "major_change" or _ => new[]
            {
                "Consider creating new method instead of modifying",
                "Deprecate old version if possible",
        ...
        foreach (var entity in entities)
        {
            ...
            if (clones.Count >= 2) // At least 2 clones (including original)
            {
                ...
                var group = new CloneGroup
                {
                    ...
                    RefactoringRecommendation = GenerateRefactoringRecommendation(
                        members.Count,
                        entity.Type
                    ),
                    Priority = CalculateRefactoringPriority(
                        members.Count,
                        clones.Average(c => c.Similarity)
                    ),
                };
    ...
    [Description(
        "Incrementally reindex modified files in the semantic search index. "
    ...
    public static async Task<object> ReindexChangedFiles(
        ...
        ISolutionManager solutionManager,
        ILogger<SemanticAnalysisToolsLogCategory> logger,
        [Description("Array of absolute file paths to reindex")] string[] filePaths,
        CancellationToken cancellationToken = default
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
