...
using UltraCode.CSharp.Tools.Infrastructure.HighPerformanceIO;
...
public static class MiscTools
{
    ...
    [
        ...
        Description(
            "Allows requesting a new tool to be added to the UltrasharpTools MCP server. Logs the request for review."
        )
    ]
    public static async Task<string> RequestNewTool(
        ...
        [Description("Expected input parameters and their descriptions.")]
                ...
                try
                {
                    ...
                    if (string.IsNullOrEmpty(logsDirectory))
                    {
                        throw new InvalidOperationException(
                            "Failed to determine logs directory path"
                        );
                    }
                    ...
                    if (!Directory.Exists(logsDirectory))
                    {
                        ...
                        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
                        {
                            logger.LogError(
                                ex,
                                "Failed to create logs directory at {LogsDirectory}",
                                logsDirectory
                            );
                            throw new McpException(
                                $"Failed to create logs directory: {ex.Message}"
                            );
                        }
                    }
                    ...
                    if (File.Exists(RequestLogFilePath))
                    {
                        try
                        {
                            ...
                            existingRequests =
                                JsonSerializer.Deserialize<List<ToolRequest>>(existingJson)
                                ?? new List<ToolRequest>();
                        }
                        catch (JsonException ex)
                        {
                            logger.LogWarning(
                                ex,
                                "Failed to deserialize existing tool requests, starting with a new list"
                            );
                        ...
                        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
                        {
                            ...
                            throw new McpException(
                                $"Failed to read existing tool requests: {ex.Message}"
                            );
                        }
                    }
                    ...
                    catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
                    {
                        logger.LogError(
                            ex,
                            "Failed to write tool requests to file at {FilePath}",
                            RequestLogFilePath
                        );
