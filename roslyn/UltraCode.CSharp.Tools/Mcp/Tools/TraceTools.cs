...
using UltraCode.CSharp.Tools.Models;
...
public static partial class TraceTools
{
    ...
    public static async Task<object> TraceExecution(
        ...
        [Description(
            "Fully qualified name of the entry point method (e.g., MyNamespace.MyClass.MyMethod)"
        )]
            string entryPointFqn,
        [Description(
            "Optional: Fully qualified name of the exit point method. Trace stops when this method is called."
        )]
            string? exitPointFqn = null,
        [Description("Maximum depth of method calls to trace. Default: 10")] int maxDepth = 10,
        ...
        [Description("Maximum number of paths to trace when traceAllPaths is true. Default: 10")]
            int maxPaths = 10,
        [Description(
            "Whether to unwrap async/await operations and show state machine transitions. Default: true"
        )]
    ...
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                {
                    throw new McpException($"maxDepth must be between 1 and 50, got: {maxDepth}");
                }
    ...
    public static async Task<object> TraceBackwards(
        ...
        ILogger<TraceToolsLogCategory> logger,
        [Description(
            "Fully qualified name of the crash/failure point method (e.g., MyNamespace.MyClass.ProblematicMethod)"
        )]
            string crashPointFqn,
        [Description(
            "Optional: Fully qualified name of the expected start/entry point. If not specified, searches for all entry points."
        )]
            string? startPointFqn = null,
        [Description(
            "Optional: Stack trace lines (from logs) to help identify the most likely path"
        )]
            List<string>? stackTraceHints = null,
        [Description("Maximum depth to search backwards. Default: 15")] int maxDepth = 15,
        [Description("Maximum number of paths to return. Default: 5")] int maxPaths = 5,
    ...
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                {
                    throw new McpException($"maxPaths must be between 1 and 20, got: {maxPaths}");
                }
    ...
    private static object FormatBacktraceResult(BacktraceResult result)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    sb.AppendLine(
                        $"Path #{path.PathId} (Confidence: {path.Confidence:P0}, Depth: {path.Depth})"
                    );
                    sb.AppendLine(new string('─', 80));
                    ...
                    foreach (var frame in path.Frames)
                    {
                        ...
                        sb.AppendLine(
                            $"{matchIndicator}[{frame.FrameNumber}] {indent}{frame.Description}"
                        );
                        ...
                        {
                            sb.AppendLine(
                                $"   {indent}   Called from: {frame.CallSite.SourceLocation}"
                            );
                            if (
                                !string.IsNullOrEmpty(frame.CallSite.CallExpression)
                                && frame.CallSite.CallExpression.Length < 100
                            )
                            {
                                sb.AppendLine(
                                    $"   {indent}   Expression: {frame.CallSite.CallExpression}"
                                );
                            }
                        }
        ...
        formatted["paths"] = result
            .CallPaths.Select(p => new
            {
                pathId = p.PathId,
    ...
    public static async Task<object> AnalyzePathFeasibility(
        ...
        ILogger<TraceToolsLogCategory> logger,
        [Description(
            "Fully qualified name of the entry point method to analyze (e.g., MyNamespace.MyClass.MyMethod)"
        )]
            string entryPointFqn,
        [Description(
            "Optional: Fully qualified name of the exit point. Analysis focuses on paths reaching this point."
        )]
            string? exitPointFqn = null,
        [Description("Maximum depth of path exploration. Default: 10")] int maxDepth = 10,
        [Description(
            "Optional: Initial constraints on input parameters (JSON dictionary mapping parameter names to constraint expressions)"
        )]
    ...
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                {
                    throw new McpException(
                        "No solution loaded. Use UltrasharpTool_LoadSolution first."
                    );
                }
