// Base MCP tool structure and some analysis methods derived from RoslynMCP by Christopher Arquiza
// https://github.com/carquiza/RoslynMCP
// Original code licensed under MIT License
// Modifications: Added FastSymbolIndex integration, enhanced error handling, performance optimizations

using DiffPlex.DiffBuilder;
using DiffPlex.DiffBuilder.Model;
using ModelContextProtocol;

namespace UltraCode.CSharp.Tools.Mcp.Tools;
...
public static partial class AnalysisTools {
    ...
    [Description(
        "Recursively lists all nested types, methods, properties, fields, and enums within a given parent type. Ideal for gaining a complete mental model of a type hierarchy at a glance."
    )]
    public static async Task<object> GetAllSubtypes(
        ISolutionManager solutionManager,
        ICodeAnalysisService codeAnalysisService,
        ILogger<AnalysisToolsLogCategory> logger,
        [Description("The fully qualified name of the parent type (e.g., MyNamespace.MyClass).")]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(GetAllSubtypes),
                    cancellationToken
                );

                logger.LogInformation(
                    "Executing {GetAllSubtypes} for: {TypeName}",
                    nameof(GetAllSubtypes),
                    fullyQualifiedParentTypeName
                );

                try {
                    var roslynSymbol = await ToolHelpers.GetRoslynNamedTypeSymbolOrThrowAsync(
                        solutionManager,
                        fullyQualifiedParentTypeName,
                        cancellationToken
                    );
                    return ToolHelpers.ToJson(
                        await BuildRoslynSubtypeTreeAsync(
                            roslynSymbol,
                            codeAnalysisService,
                            cancellationToken
                        )
                    ...
                    logger.LogDebug(
                        ex,
                        "Roslyn symbol not found for {TypeName}, trying reflection.",
                        fullyQualifiedParentTypeName
                    );
                ...
                var reflectionType = await ToolHelpers.GetReflectionTypeOrThrowAsync(
                    solutionManager,
                    fullyQualifiedParentTypeName,
                    cancellationToken
                );
                return ToolHelpers.ToJson(
                    BuildReflectionSubtypeTree(reflectionType, cancellationToken)
                );
            },
            logger,
            nameof(GetAllSubtypes),
    ...
    internal static async Task<object> BuildRoslynSubtypeTreeAsync(
        ...
        foreach (var member in typeSymbol.GetMembers()) {
            ...
            if (
                ...
                || member.Kind == SymbolKind.ErrorType
                || ToolHelpers.IsPropertyAccessor(member)
            ) {
                continue;
            }
            ...
            var memberInfo = new {
                signature = ToolHelpers.GetRoslynSymbolModifiersString(member)
                    + " "
                    + CodeAnalysisService.GetFormattedSignatureAsync(member, false),
        ...
        var orderedKinds = membersByKind
            .Keys.OrderBy(k =>
                k switch {
                    "Class" => 1,
                    "Interface" => 1,
    ...
    private static object BuildReflectionSubtypeTree(Type type, CancellationToken cancellationToken) {
        ...
        foreach (
            ...
            var memberItem = new {
                ...
                signature = ToolHelpers.GetReflectionMemberModifiersString(memberInfo)
                    + " "
                    + memberInfo.ToString(),
                members = memberInfo is Type nestedType
                    ? BuildReflectionSubtypeTree(nestedType, cancellationToken)
                    : null,
            };
    ...
    public static async Task<object> GetMembers(
        ...
        [Description(
            "If true, includes private members; otherwise, only public/internal/protected members."
        )]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                try {
                    ...
                    string typeName = ToolHelpers.RemoveGlobalPrefix(
                        roslynSymbol.ToDisplayString(ToolHelpers.FullyQualifiedFormatWithoutGlobal)
                    );
                    ...
                    foreach (var member in roslynSymbol.GetMembers()) {
                        ...
                        if (shouldInclude) {
                            try {
                                ...
                                var locationKey =
                                    location != null ? location.FilePath : defaultLocation;

                                var kind = ToolHelpers.GetSymbolKindString(member);
                                string xmlDocs =
                                    await codeAnalysisService.GetXmlDocumentationAsync(
                                        member,
                                        cancellationToken
                                    ) ?? string.Empty;
                                string signature =
                                    ToolHelpers.GetRoslynSymbolModifiersString(member)
                                    + " "
                                    + (
                                        CodeAnalysisService.GetFormattedSignatureAsync(
                                            member,
                                            false
                                        )
                                    )
                                        ...
                                        .Trim()
                                    + "//FQN: "
                                    + FuzzyFqnLookupService.GetSearchableString(member);
...
membersByKind;
                                ...
                                logger.LogWarning(
                                    ex,
                                    "Error retrieving details for member {MemberName} in type {TypeName}",
                                    member.Name,
                                    fullyQualifiedTypeName
                                );
...
membersByKind;
                    ...
                    logger.LogDebug(
                        ex,
                        "Roslyn symbol not found for {TypeName} or error occurred, trying reflection.",
                        fullyQualifiedTypeName
                    );
                    // Fall through to reflection if Roslyn symbol not found or other McpException related to Roslyn.
                }
                ...
                return GetReflectionTypeMembersAsync(
                    reflectionType,
                    includePrivateMembers,
                    logger,
                    cancellationToken
    ...
    private static object GetReflectionTypeMembersAsync(
        ...
        try {
            foreach (
                var memberInfo in reflectionType.GetMembers(
                    BindingFlags.Public
                        | BindingFlags.NonPublic
                        | BindingFlags.Instance
                        | BindingFlags.Static
                ...
                try {
                    switch (memberInfo) {
                        case FieldInfo fi:
                            ...
                            accessibilityString = ToolHelpers.GetReflectionMemberModifiersString(
                                fi
                            );
                            break;
                        case MethodBase mb:
                    ...
                    logger.LogWarning(
                        ex,
                        "Error processing reflection member {MemberName}",
                        memberInfo.Name
                    );
            ...
            logger.LogError(
                ex,
                "Error retrieving members for reflection type {TypeName}",
                reflectionType.FullName
            );
            throw new McpException(
                $"Failed to retrieve members for type '{reflectionType.FullName}': {ex.Message}"
            );
        }
    ...
    public static async Task<string> ViewDefinition(
        ...
        [Description("The fully qualified name of the symbol (type, method, property, etc.).")]
    string fullyQualifiedSymbolName,
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                logger.LogInformation(
                    "Executing '{ViewDefinition}' for: {SymbolName}",
                    nameof(ViewDefinition),
                    fullyQualifiedSymbolName
                );
                ...
                if (roslynSymbol is null) {
                    ...
                    return $"Symbol '{fullyQualifiedSymbolName}' not found in the current solution. Try `search_definitions`, `get_members`, or `list_file_entities` to find what you need.";
                }
                ...
                if (locations.Count == 0) {
                    ...
                    if (sourceResult != null) {
                        ...
                        var externalReferenceContext = roslynSymbol switch {
                            Microsoft.CodeAnalysis.INamedTypeSymbol type =>
                                await ContextInjectors.CreateTypeReferenceContextAsync(
                                    codeAnalysisService,
                                    logger,
                                    type,
                                    cancellationToken
                            ...
                            Microsoft.CodeAnalysis.IMethodSymbol method =>
                                await ContextInjectors.CreateCallGraphContextAsync(
                                    codeAnalysisService,
                                    logger,
                                    method,
                                    cancellationToken
                    ...
                    // No source found - return informational message instead of error
                    logger.LogInformation(
                        "No source definition found for '{SymbolName}' - symbol might be in metadata only",
                        fullyQualifiedSymbolName
                    );
                    return $"No source definition found for '{fullyQualifiedSymbolName}'. The symbol might be defined in metadata (compiled assembly) only and couldn't be decompiled.";
                }
                ...
                if (isPartialType) {
                    ...
                    return await HandlePartialTypeDefinitionAsync(
                        roslynSymbol,
                        solution,
                        codeAnalysisService,
                        logger,
                    ...
                    return await HandleSingleDefinitionAsync(
                        roslynSymbol,
                        solution,
                        locations.First(),
                        codeAnalysisService,
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(ListImplementations),
                    cancellationToken
                );

                logger.LogInformation(
                    "Executing '{ViewImplementations}' for: {SymbolName}",
                    nameof(ListImplementations),
                    fullyQualifiedSymbolName
                );
                ...
                try {
                            ...
                            foreach (var impl in implementingSymbols.OfType<INamedTypeSymbol>()) {
                                ...
                                implementations.Add(
                                    new {
                                        ...
                                        signature = CodeAnalysisService.GetFormattedSignatureAsync(
                                            impl,
                                            false
                                        ),
                                        fullyQualifiedName = FuzzyFqnLookupService.GetSearchableString(
                                            impl
                                        ),
                                        location = GetDeclarationLocationInfo(impl)
                                            .FirstOrDefault(),
                                    }
                                );
                            }
                            ...
                            foreach (var derived in derivedClasses) {
                                ...
                                implementations.Add(
                                    new {
                                        ...
                                        location = GetDeclarationLocationInfo(derived)
                                            .FirstOrDefault(),
                                    }
                                );
                            }
                    ...
                    throw new McpException(
                        $"Error finding implementations for symbol '{fullyQualifiedSymbolName}': {ex.Message}"
                    );
                }
    ...
    private static async Task<string> HandlePartialTypeDefinitionAsync(
        ...
        string referenceContext = await ContextInjectors.CreateTypeReferenceContextAsync(
            codeAnalysisService,
            logger,
            namedTypeSymbol,
            cancellationToken
        ...
        foreach (var syntaxRef in roslynSymbol.DeclaringSyntaxReferences) {
            ...
            if (document == null) {
                logger.LogWarning(
                    "Could not find document for partial declaration in file: {FilePath}",
            ...
            if (definitionNode == null) {
                logger.LogWarning(
                    "Could not find definition syntax for partial declaration in file: {FilePath}",
                    syntaxRef.SyntaxTree.FilePath
                );
                continue;
            }
            ...
            allFiles.Add(
                $"{filePath} (Lines {lineInfo.StartLinePosition.Line + 1} - {lineInfo.EndLinePosition.Line + 1})"
            );
            allSourceCode.Add(
                $"<code partialDefinitionFile='{filePath}' lines='{lineInfo.StartLinePosition.Line + 1} - {lineInfo.EndLinePosition.Line + 1}'>\n{result}\n</code>"
            );
        }
    ...
    private static async Task<string> HandleSingleDefinitionAsync(
        ...
        if (sourceLocation.SourceTree == null) {
            throw new McpException(
                $"Source tree not available for symbol '{roslynSymbol.ToDisplayString(ToolHelpers.FullyQualifiedFormatWithoutGlobal)}'."
            );
        }
        ...
        if (document == null) {
            throw new McpException(
                $"Could not find document for symbol '{roslynSymbol.ToDisplayString(ToolHelpers.FullyQualifiedFormatWithoutGlobal)}'."
            );
        }
        ...
        if (node == null) {
            throw new McpException(
                $"Could not find syntax node for symbol '{roslynSymbol.ToDisplayString(ToolHelpers.FullyQualifiedFormatWithoutGlobal)}'."
            );
        }
        ...
        SyntaxNode? definitionNode = node;
        if (
            roslynSymbol is Microsoft.CodeAnalysis.IMethodSymbol
            || roslynSymbol is Microsoft.CodeAnalysis.IPropertySymbol
            || roslynSymbol is Microsoft.CodeAnalysis.IFieldSymbol
            || roslynSymbol is Microsoft.CodeAnalysis.IEventSymbol
        ...
        if (definitionNode == null) {
            throw new McpException(
                $"Could not find definition syntax for symbol '{roslynSymbol.ToDisplayString(ToolHelpers.FullyQualifiedFormatWithoutGlobal)}'."
            );
        }
    ...
    private static List<LocationInfo> GetDeclarationLocationInfo(ISymbol symbol) {
        ...
        foreach (var syntaxRef in symbol.DeclaringSyntaxReferences) {
            ...
            var fullSpan = node switch {
                ...
                LocalFunctionStatementSyntax localFuncNode => localFuncNode
                    .GetLocation()
                    .GetLineSpan(),
                ...
                ConversionOperatorDeclarationSyntax convNode => convNode
                    .GetLocation()
                    .GetLineSpan(),
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(FindReferences),
                    cancellationToken
                );

                logger.LogInformation(
                    "Executing '{FindReferences}' for: {SymbolName}",
                    nameof(FindReferences),
                    fullyQualifiedSymbolName
                );
                ...
                var referencedSymbols = await codeAnalysisService.FindReferencesAsync(
                    symbol,
                    cancellationToken
                );
                ...
                try {
                        ...
                        foreach (var location in refGroup.Locations) {
                            ...
                            if (location.Document != null && location.Location.IsInSource) {
                                try {
                                    ...
                                    if (sourceTree == null) {
                                        logger.LogWarning(
                                            "Null source tree for reference location in {FilePath}",
                                    ...
                                    for (
                                        int i = Math.Max(
                                            0,
                                            lineSpan.StartLinePosition.Line - linesAround
                                        );
                                        i
                                            <= Math.Min(
                                                sourceText.Lines.Count - 1,
                                                lineSpan.EndLinePosition.Line + linesAround
                                            );
                                        i++
                                    ) {
                                        contextLines.Add(
                                            TrimLeadingWhitespace(sourceText.Lines[i].ToString())
                                        );
                                    }
                                    ...
                                    try {
                                        ...
                                        if (token.Parent != null) {
                                            ...
                                            if (memberDecl != null) {
                                                var semanticModel =
                                                    await solutionManager.GetSemanticModelAsync(
                                                        location.Document.Id,
                                                        cancellationToken
                                                    );
                                                var parentSymbol = semanticModel?.GetDeclaredSymbol(
                                                    memberDecl,
                                                    cancellationToken
                                                );
                                        ...
                                        logger.LogWarning(
                                            ex,
                                            "Error getting parent member for reference in {FilePath}",
                                            location.Document.FilePath ?? "unknown file"
                                        );
                                    }
                                    ...
                                    logger.LogWarning(
                                        ex,
                                        "Error processing reference location in {FilePath}",
                                        location.Document.FilePath ?? "unknown file"
                                    );
                                }
                            }
                    ...
                    return ToolHelpers.ToJson(
                        new {
                            ...
                            references = references
                                .OrderBy(r => ((dynamic)r).location.filePath)
                    ...
                    logger.LogError(
                        ex,
                        "Error collecting references for symbol {SymbolName}",
                        fullyQualifiedSymbolName
                    );

                    if (references.Count > 0) {
                        logger.LogInformation(
                            "Returning partial references ({Count}) for {SymbolName}",
                            references.Count,
                            fullyQualifiedSymbolName
                        );
                    ...
                    throw new McpException(
                        $"Failed to find references for symbol '{fullyQualifiedSymbolName}': {ex.Message}"
                    );
                }
            },
            logger,
    ...
    [Description(
        "Shows the inheritance hierarchy for a class or interface (base types and derived types). Essential for understanding type relationships and architecture."
    )]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(ViewInheritanceChain),
                    cancellationToken
                );

                logger.LogInformation(
                    "Executing '{ViewInheritanceChain}' for: {TypeName}",
                    nameof(ViewInheritanceChain),
                    fullyQualifiedTypeName
                );
                ...
                try {
                    ...
                    try {
                        ...
                        while (currentType != null) {
                            ...
                            baseTypes.Add(
                                new {
                                    ...
                                    location = GetDeclarationLocationInfo(currentType)
                                        .FirstOrDefault(),
                                }
                            );
                        ...
                        try {
                            foreach (var iface in roslynSymbol.Interfaces) {
                                ...
                                baseTypes.Add(
                                    new {
                                        ...
                                        location = GetDeclarationLocationInfo(iface)
                                            .FirstOrDefault(),
                                        isInterface = true,
                                    }
                                );
                            ...
                            logger.LogWarning(
                                ex,
                                "Error retrieving interfaces for type {TypeName}",
                                fullyQualifiedTypeName
                            );
                        ...
                        try {
                            if (
                                roslynSymbol.TypeKind == TypeKind.Class
                                || roslynSymbol.TypeKind == TypeKind.Interface
                            ) {
                                ...
                                foreach (var derivedSymbol in derived.OfType<INamedTypeSymbol>()) {
                                    ...
                                    derivedTypes.Add(
                                        new {
                                            ...
                                            location = GetDeclarationLocationInfo(derivedSymbol)
                                                .FirstOrDefault(),
                                        }
                                    );
                                }
                            ...
                            logger.LogWarning(
                                ex,
                                "Error retrieving derived types for {TypeName}",
                                fullyQualifiedTypeName
                            );
                            hasPartialResults = true;
                            errorMessage =
                        ...
                        return ToolHelpers.ToJson(
                            new {
                                ...
                                location = GetDeclarationLocationInfo(roslynSymbol)
                                    .FirstOrDefault(),
                                baseTypes,
                                derivedTypes,
                                hasPartialResults,
                        ...
                        logger.LogError(
                            ex,
                            "Error analyzing inheritance chain for Roslyn type {TypeName}",
                            fullyQualifiedTypeName
                        );
                ...
                try {
                    ...
                    try {
                        ...
                        while (currentBase != null && currentBase != typeof(object)) {
                            ...
                            baseTypes.Add(
                                new {
                                    ...
                                    signature = ToolHelpers.GetReflectionTypeModifiersString(
                                        currentBase
                                    )
                                        + " "
                                        + currentBase.FullName,
                        ...
                        if (
                            reflectionType.BaseType == typeof(object)
                            || (
                                reflectionType.IsClass
                                && reflectionType.BaseType == null
                        ...
                        logger.LogWarning(
                            ex,
                            "Error retrieving base types for reflection type {TypeName}",
                            fullyQualifiedTypeName
                        );
                        hasPartialResults = true;
                    ...
                    try {
                        ...
                        foreach (var iface in reflectionType.GetInterfaces()) {
                            ...
                            baseTypes.Add(
                                new {
                                    kind = "Interface",
                                    signature = ToolHelpers.GetReflectionTypeModifiersString(iface)
                                        + " "
                                        + iface.FullName,
                        ...
                        logger.LogWarning(
                            ex,
                            "Error retrieving interfaces for reflection type {TypeName}",
                            fullyQualifiedTypeName
                        );
                        hasPartialResults = true;
                        errorMessage =
                    ...
                    return ToolHelpers.ToJson(
                        new {
                            kind = reflectionType.IsInterface
                                ? "Interface"
                                : (reflectionType.IsEnum ? "Enum" : "Class"),
                            signature = ToolHelpers.GetReflectionTypeModifiersString(reflectionType)
                                + " "
                                + reflectionType.FullName,
                    ...
                    logger.LogError(
                        ex,
                        "Error analyzing inheritance chain for reflection type {TypeName}",
                        fullyQualifiedTypeName
                    );
                    throw new McpException(
                        $"Error analyzing inheritance chain via reflection: {ex.Message}"
                    );
                }
            },
            logger,
    ...
    [Description(
        "Displays methods that call a specific method (incoming) and methods called by it (outgoing). Critical for understanding control flow and method relationships across the codebase."
    )]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(ViewCallGraph),
                    cancellationToken
                );

                logger.LogInformation(
                    "Executing '{ViewCallGraph}' for: {MethodName}",
                    nameof(ViewCallGraph),
                    fullyQualifiedMethodName
                );
                ...
                try {
                    ...
                    var callers = await codeAnalysisService.FindCallersAsync(
                        methodSymbol,
                        cancellationToken
                    );
                    foreach (var callerInfo in callers) {
                        cancellationToken.ThrowIfCancellationRequested();
                        try {
                            var callLocations = callerInfo
                                .Locations.Where(l => l.IsInSource && l.SourceTree != null)
                                .Select(l => {
                                    var lineSpan = l.GetLineSpan();
                                    return new { line = lineSpan.StartLinePosition.Line + 1 };
                                })
                                .ToList();

                            incomingCalls.Add(
                                new {
                                    kind = ToolHelpers.GetSymbolKindString(
                                    ...
                                    location = GetDeclarationLocationInfo(callerInfo.CallingSymbol)
                                        .FirstOrDefault(),
                                    callLocations,
                                }
                            );
                            ...
                            logger.LogWarning(
                                ex,
                                "Error processing caller {CallerSymbol} for method {MethodName}",
                                callerInfo.CallingSymbol.Name,
                                fullyQualifiedMethodName
                            );
                            hasPartialResults = true;
                        }
                        ...
                        try {
                            outgoingCalls.Add(
                                new {
                                    ...
                                    calleeSignature = CodeAnalysisService.GetFormattedSignatureAsync(
                                        callee,
                                        false
                                    ),
                                    fullyQualifiedName = FuzzyFqnLookupService.GetSearchableString(
                            ...
                            logger.LogWarning(
                                ex,
                                "Error processing callee {CalleeSymbol} for method {MethodName}",
                                callee.Name,
                                fullyQualifiedMethodName
                            );
                            hasPartialResults = true;
                    ...
                    incomingCalls = incomingCalls.OrderBy(c => ((dynamic)c).signature).ToList();

                    outgoingCalls = outgoingCalls.OrderBy(c => ((dynamic)c).signature).ToList();
                } catch (Exception ex)
                      when (!(ex is McpException || ex is OperationCanceledException)) {
                    logger.LogWarning(
                        ex,
                        "Error finding call graph for method {MethodName}",
                        fullyQualifiedMethodName
                    );
                    hasPartialResults = true;
    ...
    public static async Task<object> SearchDefinitions(
        ...
        ILogger<AnalysisToolsLogCategory> logger,
        [Description(
            "The regex pattern to match against full declaration text (multiline) and symbol names."
        )]
            string regexPattern,
        CancellationToken cancellationToken
    ) {
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(SearchDefinitions),
                    cancellationToken
                );

                logger.LogInformation(
                    "Executing '{SearchDefinitions}' with pattern: {RegexPattern}",
                    nameof(SearchDefinitions),
                    regexPattern
                );
                ...
                using var earlyStopCts = CancellationTokenSource.CreateLinkedTokenSource(
                    cancellationToken
                );

                try {
                    var projectTasks = solutionManager
                        .GetProjects()
                        .Select(project =>
                            Task.Run(
                                async () => {
                                    try {
                                        ...
                                        if (compilation == null) {
                                            ...
                                            logger.LogWarning(
                                                "Skipping project {ProjectName}, compilation is null",
                                                project.Name
                                            );
                                            return;
                                        }
                                        ...
                                        foreach (var syntaxTree in compilation.SyntaxTrees) {
                                            try {
                                                ...
                                                var matchedNodesInFile =
                                                    new Dictionary<SyntaxNode, List<Match>>();
                                                ...
                                                foreach (
                                                    ...
                                                    try {
                                                        ...
                                                        if (declMatches.Count > 0) {
                                                            matchedNodesInFile.Add(
                                                                node,
                                                                declMatches.Cast<Match>().ToList()
                                                            );
                                                            if (
                                                                !matchedNodeSpans.TryGetValue(
                                                                    filePath,
                                                                    out var spans
                                                                )
                                                            ) {
                                                        ...
                                                        logger.LogTrace(
                                                            ex,
                                                            "Error examining node in {FilePath}",
                                                            filePath
                                                        );
                                                        hasPartialResults = true;
                                                    }
                                                ...
                                                foreach (
                                                    var (node, nodeMatches) in matchedNodesInFile
                                                ) {
                                                    // Check if we've already exceeded the result limit
                                                    if (
                                                        Interlocked.CompareExchange(
                                                    ...
                                                    try {
                                                        ...
                                                        if (symbol != null) {
                                                            ...
                                                            if (containingType != null) {
                                                                ...
                                                                parentFqn =
                                                                    FuzzyFqnLookupService.GetSearchableString(
                                                                        containingType
                                                                    );
                                                            ...
                                                            foreach (Match match in nodeMatches) {
                                                                ...
                                                                int matchLineNumber =
                                                                    matchLinePos.Line + 1;
                                                                var matchLine = sourceText
                                                                    .Lines[matchLinePos.Line]
                                                                    .ToString()
                                                                    .Trim();
                                                        ...
                                                        logger.LogTrace(
                                                            ex,
                                                            "Error processing syntax node in {FilePath}",
                                                            filePath
                                                        );
                                                        hasPartialResults = true;
                                                    }
                                                ...
                                                logger.LogWarning(
                                                    ex,
                                                    "Error processing syntax tree {FilePath}",
                                                    syntaxTree.FilePath ?? "unknown file"
                                                );
                                                hasPartialResults = true;
                                            }
                                        ...
                                        hasPartialResults = true;
                                        errors.Add(
                                            $"Error in project {project.Name}: {ex.Message}"
                                        );
                                    }
                                },
                                earlyStopCts.Token
                    ...
                    logger.LogError(
                        ex,
                        "Error searching Roslyn symbols with pattern {Pattern}",
                        regexPattern
                    );
                    hasPartialResults = true;
                ...
                var reflectionSearchTask = Task.Run(
                    async () => {
                        try {
                            // If already at limit, skip reflection search
                            if (
                                Interlocked.CompareExchange(ref totalMatchesFound, 0, 0)
                                >= MaxSearchResults
                            ) {
                                hasPartialResults = true;
                                earlyStopCts.Cancel(); // Stop other parallel tasks
                            ...
                            string reflectionPattern = ClassRegex()
                                .Replace(regexPattern, string.Empty);

                            var reflectionRegex = new Regex(
                                reflectionPattern,
                                            ...
                                            () => {
                                                for (
                                                    ...
                                                    try {
                                                        ...
                                                        var matchedMembers =
                                                            new List<(string name, object match)>();
                                                        ...
                                                        foreach (
                                                            ...
                                                            if (
                                                                ...
                                                                if (memberInfo is MethodInfo method) {
                                                                    ...
                                                                    signature =
                                                                        $"{type.FullName}.{memberInfo.Name}({parameters})";
                                                                }
                                                                ...
                                                                if (
                                                                    ...
                                                                    matches.Add(
                                                                        new {
                                                                            kind = ToolHelpers.GetReflectionMemberTypeKindString(
                                                                                memberInfo
                                                                            ),
                                                                            parentFqn = type.FullName
                                                                                ?? type.Name,
                                                                            signature = signature,
                                                        ...
                                                        if (
                                                            ...
                                                            if (
                                                                ...
                                                                matches.Add(
                                                                    new {
                                                                        kind = ToolHelpers.GetReflectionTypeKindString(
                                                                            type
                                                                        ),
                                                                        parentFqn = type.Namespace
                                                                            ?? "global",
                                                        ...
                                                        logger.LogWarning(
                                                            ex,
                                                            "Error processing reflection type {TypeName}",
                                                            type.FullName ?? type.Name
                                                        );
                                                        hasPartialResults = true;
                                                    }
                            ...
                            logger.LogError(
                                ex,
                                "Error searching reflection members with pattern {Pattern}",
                                regexPattern
                            );
                            hasPartialResults = true;
                ...
                try {
                    await Task.WhenAny(
                        reflectionSearchTask,
                        Task.Delay(TimeSpan.FromSeconds(5), cancellationToken)
                    );
                    if (!reflectionSearchTask.IsCompleted) {
                        logger.LogWarning(
                            "Reflection search timed out after 5 seconds. Returning partial results."
                        );
                        hasPartialResults = true;
                        errors.Add(
                            "Reflection search timed out after 5 seconds, returning partial results."
                          ...
                          reflectionSearchTask.IsFaulted
                          && reflectionSearchTask.Exception != null
                      ) {
                        throw reflectionSearchTask.Exception.InnerException
                            ?? reflectionSearchTask.Exception;
                    }
                ...
                var groupedMatches = matches
                    ...
                    .ToDictionary(
                        g => g.Key,
                        g =>
                            g.GroupBy(m => ((dynamic)m).parentFqn)
                                .ToDictionary(
                                    pg => ToolHelpers.RemoveGlobalPrefix(pg.Key),
                                    pg =>
                                        pg.GroupBy(m => ((dynamic)m).kind)
                                            .ToDictionary(
                                                kg => kg.Key,
                                                kg =>
                                                    kg.Where(m =>
                                                            !IsGeneratedCode(((dynamic)m).match)
                                                        )
                                                        .DistinctBy(m => ((dynamic)m).match)
                                                        .Select(m => new {
                                                            match = ((dynamic)m).match,
                                                            line = ((dynamic)m).location.line > 0
                                                                ? ((dynamic)m).location.line
                                                                : null,
                                                        })
                                                        .OrderBy(m => m.line)
                                                        .ToList()
                ...
                if (hasPartialResults) {
                    resultsLimitMessage =
                        $"Some search results omitted for brevity, try narrowing your search if you didn't find what you needed.";
                ...
                return ToolHelpers.ToJson(
                    new {
                        ...
                        resultsLimitMessage,
                        errors = errors.Count() > 0 ? errors.ToList() : null,
                        totalMatchesFound,
                    }
                );
            },
    ...
    public static async Task<object> ManageUsings(
        ...
        [Description(
            "For 'read', must be 'None'. For 'write', provide all using directives that should exist in the file. This will replace all existing usings."
        )]
            string codeToWrite,
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                if (
                    operation == "write"
                    && (codeToWrite == "None" || string.IsNullOrEmpty(codeToWrite))
                ) {
                    throw new McpException(
                        "For write operations, codeToWrite must contain the complete list of using directives"
                    );
                }
                ...
                var solution =
                    ...
                    ?? throw new McpException("Current solution is null.");

                var document =
                    solution
                        .Projects.SelectMany(p => p.Documents)
                        .FirstOrDefault(d => d.FilePath == filePath)
                    ?? throw new McpException($"File '{filePath}' not found in solution.");
                ...
                var globalUsingsFile = document.Project.Documents.FirstOrDefault(d =>
                    d.Name.Equals("GlobalUsings.cs", StringComparison.OrdinalIgnoreCase)
                );
                ...
                if (operation == "read") {
                    ...
                    if (globalUsingsFile != null) {
                        var globalRoot = await globalUsingsFile.GetSyntaxRootAsync(
                            cancellationToken
                        );
                        if (globalRoot != null) {
                            globalUsings = globalRoot
                                .DescendantNodes()
                                .OfType<UsingDirectiveSyntax>()
                                .Select(u => u.ToFullString().Trim())
                                .ToList();
                ...
                // Write operation
                bool isGlobalUsings = document.FilePath!.EndsWith(
                    "GlobalUsings.cs",
                    StringComparison.OrdinalIgnoreCase
                );

                // Parse and normalize directives
                var directives = codeToWrite
                    .Split('\n')
                    .Select(line => line.Trim())
                    .Where(line => !string.IsNullOrWhiteSpace(line))
                    .Select(line =>
                        isGlobalUsings && !line.StartsWith("global ") ? $"global {line}" : line
                    )
                    .ToList();
                ...
                try {
                    ...
                    newRoot = isGlobalUsings
                        ...
                        : ((CompilationUnitSyntax)root).WithUsings(
                            SyntaxFactory.List(
                                CSharpSyntaxTree
                                    .ParseText(tempCode + "\nnamespace N { class C { } }")
                                    .GetRoot()
    ...
    public static async Task<object> ManageAttributes(
        ...
        [Description("'read' or 'write'. For 'read', set codeToWrite to 'None'.")] string operation,
        [Description(
            "For 'read', must be 'None'. For 'write', specify all attributes that should exist on the target declaration. This will replace all existing attributes."
        )]
            string codeToWrite,
        [Description("The FQN of the target declaration to manage attributes for")]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                if (operation == "write" && codeToWrite == "None") {
                    throw new McpException(
                        "For write operations, codeToWrite must contain the attributes to set"
                    );
                }
                ...
                if (!symbol.DeclaringSyntaxReferences.Any()) {
                    throw new McpException(
                        $"Symbol '{targetDeclaration}' has no declaring syntax references."
                    );
                }
                ...
                try {
                    ...
                    var tempCode =
                        $"{(codeToWrite.Length == 0 ? "" : codeToWrite + "\n")}public class C {{ }}";
                    newAttributeLists = CSharpSyntaxTree
                        .ParseText(tempCode)
                        .GetRoot()
                        .DescendantNodes()
                ...
                var document =
                    solutionManager.CurrentSolution?.GetDocument(syntaxRef.SyntaxTree)
                ...
                var formatted = await modificationService.FormatDocumentAsync(
                    newSolution.GetDocument(document.Id)!,
                    cancellationToken
                );
                await modificationService.ApplyChangesAsync(
                    formatted.Project.Solution,
    ...
    public static async Task<string> AnalyzeComplexity(
        ...
        [Description("The fully qualified name of the method/class, or project name to analyze")]
            string target,
        CancellationToken cancellationToken
    ) {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                switch (scope.ToLower()) {
                    case "method":
                        ...
                        await complexityAnalysisService.AnalyzeMethodAsync(
                            methodSymbol,
                            metrics,
                            recommendations,
                            cancellationToken
                    ...
                    case "class":
                        ...
                        if (typeSymbol == null)
                            throw new McpException(
                                $"Target '{target}' is not a class or interface."
                            );

                        await complexityAnalysisService.AnalyzeTypeAsync(
                            typeSymbol,
                            metrics,
                            recommendations,
                            false,
                    ...
                    case "project":
                        ...
                        await complexityAnalysisService.AnalyzeProjectAsync(
                            project,
                            metrics,
                            recommendations,
                            false,
    ...
    [Description(
        "Finds groups of semantically similar methods within the solution based on a similarity threshold."
    )]
    public static async Task<string> FindPotentialDuplicates(
        ...
        ILogger<AnalysisToolsLogCategory> logger,
        [Description(
            "The minimum similarity score (0.0 to 1.0) for methods to be considered similar. (start with 0.75)"
        )]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(
                    solutionManager,
                    logger,
                    nameof(FindPotentialDuplicates),
                    cancellationToken
                );
                logger.LogInformation(
                    "Executing '{ToolName}' with threshold {Threshold}",
                    nameof(FindPotentialDuplicates),
                    similarityThreshold
                );
                ...
                cancellationToken = CancellationTokenSource
                    ...
                    .Token;
                var similarityResults = await semanticSimilarityService.FindSimilarMethodsAsync(
                    similarityThreshold,
                    cancellationToken
                );
