...
using UltraCode.CSharp.Tools.Versioning;
...
public static partial class SnapshotTools
{
    ...
    [Description(
        "Create a snapshot of current code state. Snapshots allow safe experimentation with multiple restore points. "
    ...
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                var snapshotId = await versionManager.CreateSnapshotAsync(
                    description,
                    files,
                    cancellationToken
                );
    ...
    [Description(
        "Rollback code to a previous snapshot. This will restore files from the snapshot, overwriting current changes. "
    ...
    public static async Task<object> RollbackSnapshot(
        VersionManager versionManager,
        ILogger<SnapshotToolsLogCategory> logger,
        [Description("Snapshot ID to rollback to (from list_snapshots)")] string snapshotId,
        CancellationToken cancellationToken = default
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
    ...
    [Description(
        "List available code snapshots. Shows snapshot ID, description, creation time, file count, and backend type. "
    ...
    public static async Task<object> ListSnapshots(
        VersionManager versionManager,
        ILogger<SnapshotToolsLogCategory> logger,
        [Description("Maximum number of snapshots to return (default: 20)")] int limit = 20,
        CancellationToken cancellationToken = default
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
    ...
    [Description(
        "Cleanup old snapshots to save disk space. Keeps most recent snapshots and deletes older ones. "
    ...
    public static async Task<object> CleanupSnapshots(
        VersionManager versionManager,
        ILogger<SnapshotToolsLogCategory> logger,
        [Description("Number of most recent snapshots to keep (default: 20)")] int keepCount = 20,
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                logger.LogInformation(
                    "Cleaning up snapshots (keepCount={KeepCount}, olderThanDays={OlderThanDays})",
                    keepCount,
                    olderThanDays
                );
                ...
                var beforeCount = (
                    await versionManager.ListSnapshotsAsync(int.MaxValue, cancellationToken)
                ).Count;

                // Cleanup
                var olderThan = olderThanDays.HasValue
                    ? TimeSpan.FromDays(olderThanDays.Value)
                    : (TimeSpan?)null;
                await versionManager.CleanupOldSnapshotsAsync(
                    keepCount,
                    olderThan,
                    cancellationToken
                );
