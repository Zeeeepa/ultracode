// Base solution loading and navigation structure derived from RoslynMCP by Christopher Arquiza
...
public static class SolutionTools {
                ...
                try {
                    foreach (var project in solutionManager.GetProjects()) {
                        ...
                        try {
                                ...
                                if (solutionManager.CurrentSolution != null) {
                                    projectRefs = project
                                        .ProjectReferences.Select(pr =>
                                            solutionManager
                                                .CurrentSolution.GetProject(pr.ProjectId)
                                                ?.Name
                                        )
                            ...
                            var namespaceTreeBuilder =
                                ObjectPoolProvider.Instance.GetStringBuilder();
    ...
    public static string ExtractTargetFrameworkFromProjectFile(string projectFilePath) {
        try {
            ...
            var targetFrameworkProfile = xDoc.Descendants("TargetFrameworkProfile")
                .FirstOrDefault()
                ?.Value?.Trim();
            var targetFrameworkIdentifier = xDoc.Descendants("TargetFrameworkIdentifier")
                .FirstOrDefault()
                ?.Value?.Trim();
    ...
    [Description(
        $"Use this immediately after {nameof(LoadSolution)}. This injects a comprehensive understanding of the project structure into your context."
    )]
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () => {
                ...
                try {
                    ...
                    throw new McpException(
                        $"Error getting compilation for project '{project.Name}': {ex.Message}"
                    );
                }
                ...
                try {
                            ...
                            foreach (
                                ...
                                try {
                                    if (
                                        ...
                                        if (
                                            !allTypesByNamespace.TryGetValue(
                                                nsName,
                                                out var allTypeList
                                            )
                                        ) {
                    ...
                    var derivedTypesByNamespace = await CollectDerivedAndImplementedCounts(
                        allNamespaceContents,
                        codeAnalysisService,
                        logger,
                        cancellationToken
                    );
                    var commonImplementationInfo = new CommonImplementationInfo(
                    ...
                    try {
                        ...
                        while (
                            ...
                            var sortedNamespaces = namespaceContents
                                .Keys.OrderBy(ns => ns)
                                .ToList();
                            var namespaceParts = BuildNamespaceHierarchy(
                                sortedNamespaces,
                                namespaceContents,
                                logger
                            );
                            var rootNamespaces = namespaceParts
                                .Keys.Where(ns => ns.IndexOf('.') == -1)
                                .OrderBy(n => n)
                                .ToList();
    ...
    private static async Task<
        Dictionary<INamedTypeSymbol, Dictionary<string, List<INamedTypeSymbol>>>
    > CollectDerivedAndImplementedCounts(
        Dictionary<string, List<INamedTypeSymbol>> namespaceContents,
        ICodeAnalysisService codeAnalysisService,
        ILogger<SolutionToolsLogCategory> logger,
        CancellationToken cancellationToken
    ...
    private class CommonImplementationInfo {
        ...
        public CommonImplementationInfo(
            Dictionary<
                INamedTypeSymbol,
                Dictionary<string, List<INamedTypeSymbol>>
            > derivedTypesByNamespace
    ...
    private static bool AppendMemberInfo(
        ...
        try {
            ...
            var fields = publicOrInternalMembers
                .OfType<IFieldSymbol>()
                .Where(f =>
                ...
                .ToList();
            var constants = publicOrInternalMembers
                .OfType<IFieldSymbol>()
                .Where(f => f.IsConst)
                .ToList();
            var enumValues =
            ...
            var methods = publicOrInternalMembers
