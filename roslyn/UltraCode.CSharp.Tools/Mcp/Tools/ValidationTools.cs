...
public static partial class ValidationTools
{
    ...
    [Description(
        "Validate C# file with Roslyn analyzers. Returns diagnostics (errors, warnings, info) with locations. "
    ...
    public static async Task<object> ValidateFile(
        ...
        [Description("Absolute path to .cs file to validate")] string filePath,
            ...
            async () =>
            {
                ...
                if (
                    !Enum.TryParse<DiagnosticSeverity>(
                        minSeverity,
                        ignoreCase: true,
                        out var minSeverityEnum
                    )
                )
                {
                    throw new McpException(
                        $"Invalid severity: {minSeverity}. Valid values: Error, Warning, Info, Hidden"
                    );
                }

                // Get document
                var document = await GetDocumentByPathAsync(
                    solutionManager,
                    filePath,
                    cancellationToken
                );
    ...
    [Description(
        "Batch validate all .cs files in directory with Roslyn analyzers. Processes files in parallel for performance. "
            + "Returns aggregated statistics and per-file reports."
    )]
    public static async Task<object> ValidateDirectory(
        ISolutionManager solutionManager,
        ILogger<ValidationToolsLogCategory> logger,
        [Description("Directory path to validate (absolute)")] string directoryPath,
        [Description("Search subdirectories (default: true)")] bool recursive = true,
        [Description("Minimum severity to report (Error, Warning, Info, Hidden)")]
            string minSeverity = "Warning",
        [Description("Maximum files to process (1-1000, default: 100)")] int maxFiles = 100,
        CancellationToken cancellationToken = default
    )
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            async () =>
            {
                ...
                logger.LogInformation(
                    "Validating directory: {Directory} (recursive={Recursive}, maxFiles={MaxFiles})",
                    directoryPath,
                    recursive,
                    maxFiles
                );
                ...
                var files = Directory
                    .GetFiles(directoryPath, "*.cs", searchOption)
                    .Take(maxFiles)
                    .ToList();
                    ...
                    var batchTasks = batch.Select<string, Task<object>>(async file =>
                    {
                        try
                        {
                            ...
                            return new
                            {
                                filePath = file,
                                status = "validated",
                                errors = diagnostics.Count(d =>
                                    d.Severity == DiagnosticSeverity.Error
                                ),
                                warnings = diagnostics.Count(d =>
                                    d.Severity == DiagnosticSeverity.Warning
                                ),
                                info = diagnostics.Count(d =>
    ...
    [Description(
        "Compare validation results before and after code changes. Useful for measuring code quality improvement. "
            + "Pass results from validate_file tool to track errors fixed/introduced."
    )]
    public static Task<object> CompareValidation(
        ILogger<ValidationToolsLogCategory> logger,
        [Description("Errors count before changes")] int errorsBefore,
        [Description("Warnings count before changes")] int warningsBefore,
        [Description("Errors count after changes")] int errorsAfter,
        [Description("Warnings count after changes")] int warningsAfter,
        CancellationToken cancellationToken = default
    )
    {
        return ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(
            ...
            {
                logger.LogInformation(
                    "Comparing validation results: before({Eb}/{Wb}) vs after({Ea}/{Wa})",
                    errorsBefore,
                    warningsBefore,
                    errorsAfter,
                    warningsAfter
                ...
                var result = new
                {
                    before = new
                    {
                        ...
                        total = totalProblemsBefore,
                    },
                    after = new
                    {
                        errors = errorsAfter,
                        warnings = warningsAfter,
                        total = totalProblemsAfter,
                    },
                    changes = new
                    {
                    ...
                    status,
                    message = GenerateComparisonMessage(
                        status,
                        netChange,
                        errorsFixed,
                        warningsFixed
    ...
    private static string GenerateComparisonMessage(
        string status,
        int netChange,
        int errorsFixed,
        int warningsFixed
    )
    {
        ...
        {
            "improved" =>
                $"✅ Code quality improved! Fixed {errorsFixed} errors and {warningsFixed} warnings (net: {-netChange} problems resolved)",
            "degraded" =>
                $"⚠️ Code quality degraded. {Math.Abs(netChange)} new problems introduced",
