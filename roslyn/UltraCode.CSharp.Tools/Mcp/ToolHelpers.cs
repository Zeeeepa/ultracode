...
using UltraCode.CSharp.Tools.Serialization;

namespace UltraCode.CSharp.Tools.Mcp;
...
internal static class ToolHelpers
{
    ...
    public static void EnsureSolutionLoaded(ISolutionManager solutionManager)
    {
        if (!solutionManager.IsSolutionLoaded)
        {
            throw new McpException(
                $"No solution is currently loaded. Please use '{SharpToolPrefix}{nameof(Tools.SolutionTools.LoadSolution)}' first."
            );
        }
    }
    ...
    )
    {
        if (!solutionManager.IsSolutionLoaded)
        {
            ...
            throw new McpException(
                $"No solution is currently loaded. Please use '{SharpToolPrefix}{nameof(Tools.SolutionTools.LoadSolution)}' before calling '{operationName}'."
            );
        }
    }
    ...
    /// </summary>
    public static async Task EnsureSolutionLoadedOrAutoLoadAsync(
        ISolutionManager solutionManager,
        ILogger logger,
        string operationName,
    ...
    )
    {
        ...
        {
            logger.LogDebug(
                "No solution loaded, attempting auto-discovery for operation: {Operation}",
                operationName
            );
            ...
            {
                throw new McpException(
                    $"No solution is currently loaded and auto-discovery failed. Please use '{SharpToolPrefix}{nameof(Tools.SolutionTools.LoadSolution)}' before calling '{operationName}', or ensure you're running from a directory containing a .sln or .csproj file."
                );
            }

            logger.LogInformation(
                "Auto-loaded solution/project for operation: {Operation}",
                operationName
            );
        }
    }
    ...
    private const string FqnHelpMessage =
        $" Try `{ToolHelpers.SharpToolPrefix}{nameof(Tools.AnalysisTools.SearchDefinitions)}`, `{ToolHelpers.SharpToolPrefix}{nameof(Tools.AnalysisTools.GetMembers)}`, or `{ToolHelpers.SharpToolPrefix}{nameof(Tools.DocumentTools.ReadTypesFromRoslynDocument)}`  to find what you need.";
    ...
    )
    {
        try
        {
            var symbol = await solutionManager.FindRoslynSymbolAsync(
        ...
        {
            throw new McpException(
                $"Error finding Roslyn symbol '{fullyQualifiedSymbolName}': {ex.Message}"
            );
        }
    }
    ...
    )
    {
        try
        {
            var symbol = await solutionManager.FindRoslynNamedTypeSymbolAsync(
    ...
    )
    {
        try
        {
            var type = await solutionManager.FindReflectionTypeAsync(
                fullyQualifiedTypeName,
                cancellationToken
            );
            return type
        ...
        {
            throw new McpException(
                $"Error finding reflection type '{fullyQualifiedTypeName}': {ex.Message}"
            );
        }
    }
    ...
    public static Document GetDocumentFromSyntaxNodeOrThrow(Solution solution, SyntaxNode node)
    {
        try
        {
            ...
            return document
                ?? throw new McpException("Could not find document for the given syntax node.");
        }
        catch (Exception ex) when (!(ex is McpException))
        {
    ...
    public static string GetReflectionMemberModifiersString(MemberInfo memberInfo)
    {
        ...
        string accessibility = memberInfo switch
        {
            ...
            PropertyInfo pi => ReflectionAccessibilityToString(
                pi.GetAccessors(true).FirstOrDefault()
            ),
        ...
        switch (memberInfo)
        {
            case MethodInfo mi:
                ...
                if (mi.IsAbstract)
                    parts.Add("abstract");
                if (mi.IsVirtual && !mi.IsFinal && !mi.IsAbstract)
                    parts.Add("virtual");
                ...
                else
                {
                    ...
                    try
                    {
                        if (mi.GetBaseDefinition() != mi && !mi.IsVirtual)
                ...
                if (
                    (mi.MethodImplementationFlags & MethodImplAttributes.InternalCall) != 0
            ...
            case PropertyInfo pi:
                ...
                if (accessor != null)
                {
                    ...
                    if (accessor.IsAbstract)
                        parts.Add("abstract");
                    if (accessor.IsVirtual && !accessor.IsFinal && !accessor.IsAbstract)
                        parts.Add("virtual");
                    ...
                    else
                    {
                        ...
                        try
                        {
                            if (accessor.GetBaseDefinition() != accessor && !accessor.IsVirtual)
                                parts.Add("override");
            ...
            case EventInfo ei:
                ...
                if (addAccessor != null)
                {
                    ...
                    if (addAccessor.IsAbstract)
                        parts.Add("abstract");
                    if (addAccessor.IsVirtual && !addAccessor.IsFinal && !addAccessor.IsAbstract)
                        parts.Add("virtual");
                    ...
                    else
                    {
                        ...
                        {
                            if (
                                addAccessor.GetBaseDefinition() != addAccessor
                                && !addAccessor.IsVirtual
                            )
                                parts.Add("override");
    ...
    public static string GetReflectionMemberTypeKindString(MemberInfo memberInfo)
    {
        return memberInfo.MemberType switch
        {
            ...
            MemberTypes.TypeInfo or MemberTypes.NestedType when memberInfo is Type t =>
                GetReflectionTypeKindString(t),
    ...
    /// </summary>
    public static SymbolDisplayFormat FullyQualifiedFormatWithoutGlobal =>
