...
const branchAByPath = new Map(
Array.from(branchAUnits.values()).map((u) => [u.filePath, u]),
);
const branchBByPath = new Map(
Array.from(branchBUnits.values()).map((u) => [u.filePath, u]),
);
...
matches: Array<{ baseUnit: CodeUnit; branchAUnit: CodeUnit; branchBUnit: CodeUnit }>,
) {
...
// Fast Path matching
const { matched, unmatchedA, unmatchedB } = await this.fastPathMatch(
baseUnits,
branchAUnits,
branchBUnits,
);
...
// Semantic matching
const semanticMatches = await this.semanticMatch(unmatchedA, unmatchedB);
...
console.log(
`   Embedding generation: ${(this.metrics.embeddingGenerationTime / 1000).toFixed(2)}s`,
);
console.log(
`   Conflict detection: ${(this.metrics.conflictDetectionTime / 1000).toFixed(2)}s\n`,
);
...
const fastPathPercent =
(this.metrics.fastPathMatches / this.metrics.totalFiles) * 100 || 0;
const semanticPercent =
(this.metrics.semanticMatches / this.metrics.totalFiles) * 100 || 0;
